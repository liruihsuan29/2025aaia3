//week08-1.cpp leetcode 學習計畫 matrix 矩陣第四題
//73. Set Matrix Zeroes 設定一堆0
//要小心 不能一邊讀 一邊設0 要兩階段 先讀完 再設0
class Solution {
public:
        ...
        for(int i=0; i<matrix.size();i++){
            for(int j=0; j<matrix[0].size(); j++){
                if(matrix[i][j]==0){ //要記下對應的 i j
                    markI[i] = 1; // 標記一下 i 的這一橫條 等一下都要清為0
                    markJ[j] = 1; // 標記一下 j 的那一直條 等一下都要清為0

                }
            }
        }
        //地2階段 再照著 markI 和 markJ 的標記 把整條都清為0
        for(int i=0; i<M; i++){
            for(int j=0; j<N; j++){
